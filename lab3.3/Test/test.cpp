...
typedef Polynomial_3 P;

P poly(int deg, std::initializer_list<double> f_list);
P* new_poly(int deg, std::initializer_list<double> f_list);
...
TEST(CopyAndSwap, MoveConstructor) {
	...
	EXPECT_TRUE(q_2 == old_p_2);
	EXPECT_TRUE(p_2 == P());

	// 3. Polynomial 2
	P p_3 = poly(5, { 1, 3, 4, 9, 7, 1 });
	P old_p_3 = p_3;

	P q_3(std::move(p_3));

	EXPECT_TRUE(q_3 == old_p_3);
	EXPECT_TRUE(p_3 == P());
}
